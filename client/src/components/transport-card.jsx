      ...
      <div className={`absolute inset-0 flex flex-col items-center justify-center p-4 transition-opacity duration-300 ${isHovered ? 'opacity-100' : 'opacity-0'}`}>
        ...
        <div className="space-y-3 w-full max-w-xs">
          <Button
            ...
            className="w-full bg-kumbh-orange text-white hover:bg-kumbh-deep transition-colors py-3 text-lg font-semibold"
            data-testid={`button-${testId}-to-city`}
          >
            🚗 To the City
            <span className="block text-sm font-devanagari">शहर तक</span>
          </Button>
          ...
          >
            🚌 In the City
            <span className="block text-sm font-devanagari">शहर में</span>
          </Button>
        </div>
      </div>
    </div>
